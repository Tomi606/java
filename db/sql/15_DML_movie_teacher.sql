...
# 영화관의 전체 좌석수와 전체 상영관 수를 현재 데이터 기준으로 업데이트하는 쿼리
update theater
set th_seat = (select sum(sc_seat) from screen where sc_th_num = 1),
	th_screen = (select count(sc_num) from screen where sc_th_num = 1)
where th_num = 1;
select count(sc_seat) from screen where sc_th_num = 1;
select * from theater;

# 폴킹 감독이 웡카 영화(th_num = 1)에 감독으로 참여하는 쿼리
-- ?(mp_num) : 폴킹 감독의 mp 번호를 검색을 통해 알고 싶을 때
insert into `join` 
...
-- 검색 쿼리 작성(mp_num을 가져오는 쿼리) 후 위에 복붙
