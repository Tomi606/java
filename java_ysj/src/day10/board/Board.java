...
public class Board {
	...
	private static int count = 0;	//게시글 번호 증가
	...
	//기능 : 게시글정보를 출력(목록조회), 게시글 정보를 출력(상세조회), getter and setter, 게시글 수정
	...
	//생성자 : 게시글 번호, 제목, 내용, 작성자, 게시일이 주어졌을때 처리하는 생성자 필요
	public Board(String title, String contents, String writer, String date) {
		super();
		this.num = ++count;	//인스턴스를 생성할때마다 count값 증가시킴, 모든 인스턴스랑 공유하기 때문
		this.title = title;
		this.contents = contents;
		this.writer = writer;
		this.date = date;
